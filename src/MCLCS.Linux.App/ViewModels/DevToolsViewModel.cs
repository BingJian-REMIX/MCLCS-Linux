...
/// <summary>
/// 开发工具面板（对齐 WPF DevToolsView）：Mod 开发环境骨架生成、资源包创建器、命令语法表。
/// </summary>
public class DevToolsViewModel : ObservableObject
{
    ...
    public string Mode
    {
        ...
        set
        {
            ...
            {
                OnPropertyChanged(nameof(ShowCommands));
                OnPropertyChanged(nameof(ShowMod));
                OnPropertyChanged(nameof(ShowResourcePack));
            }
        }
    }
    ...
    {
        get => _commandFilter;
        set { if (SetField(ref _commandFilter, value)) ApplyFilter(); }
    }
    ...
    public ICommand GenerateModCommand => new RelayCommand(_ => _ = GenerateModAsync());
    public ICommand GenerateRpCommand => new RelayCommand(_ => _ = GenerateResourcePackAsync());
    public ICommand SetModeCommand => new RelayCommand(p => Mode = p as string ?? "commands");
    ...
    {
        if (string.IsNullOrWhiteSpace(ModName)) { Status = "请输入 Mod 名称"; return; }
        var pkg = string.IsNullOrWhiteSpace(ModPackage)
            ? $"com.example.{ModName.ToLowerInvariant()}"
            : ModPackage;

        var dir = await UIService.PickFolderAsync("选择 Mod 项目输出目录");
        ...
        try
        {
            ...
            await File.WriteAllTextAsync(Path.Combine(projDir, "build.gradle"),
                "plugins {\n" +
                ...
                "    mappings 'net.fabricmc:yarn:{ModGameVersion}+build.1'\n" +
                "    modImplementation 'net.fabricmc:fabric-loader:0.15.0'\n" +
                "    modImplementation 'net.fabricmc.fabric-api:fabric-api:0.95.0'\n" +
                "}\n");

            await File.WriteAllTextAsync(Path.Combine(projDir, "src", "main", "resources", "fabric.mod.json"),
                "{\n" +
                $"  \"schemaVersion\": 1,\n" +
            ...
            Status = $"Mod 项目已生成到 {projDir}";
            Services.ToastService.Show("开发工具", $"已生成 {ModName} 项目骨架", ToastKind.Success);
        }
        catch (Exception ex) { Status = $"生成失败：{ex.Message}"; }
    }

    private async Task GenerateResourcePackAsync()
    {
        if (string.IsNullOrWhiteSpace(RpName)) { Status = "请输入资源包名称"; return; }
        var dir = await UIService.PickFolderAsync("选择资源包输出目录");
        if (string.IsNullOrWhiteSpace(dir)) return;
        ...
        try
        {
            ...
            await File.WriteAllTextAsync(Path.Combine(packDir, "pack.mcmeta"),
                "{\n" +
                "  \"pack\": {\n" +
            ...
            await File.WriteAllTextAsync(Path.Combine(packDir, "pack.png"), "");
            Status = $"资源包已生成到 {packDir}";
            Services.ToastService.Show("开发工具", $"已生成 {RpName} 资源包", ToastKind.Success);
        }
        catch (Exception ex) { Status = $"生成失败：{ex.Message}"; }
    }
}
