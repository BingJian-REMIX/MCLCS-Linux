...
/// <summary>
/// Minecraft 64×64 皮肤布局的 36 面 UV 映射。
/// <para>
/// 坐标与 <see cref="Controls.SkinModel3D"/> 的标准布局一致（身体 / 四肢正面在 y20-32 带，
/// 左臂左腿在 y48-64 带），保证编辑器产出的皮肤在 3D 预览中纹理精确对齐。
/// 注：WPF 原版 SkinLayout 身体 / 四肢 y 统一偏上 4px、且身体左右面写反，此处已修正。
/// </para>
/// </summary>
public static class SkinLayout
{
    ...
    static SkinLayout()
    {
        ...
        AddFace(body, "右面", 28, 20, 4, 12);
        AddFace(body, "左面", 16, 20, 4, 12);
        ...
        AddFace(la, "右面", 32, 48, 4, 12, mirror: "右臂·左面");
        AddFace(la, "左面", 40, 48, 4, 12, mirror: "右臂·右面");
        AddFace(la, "顶面", 36, 44, 4, 4,  mirror: "右臂·顶面");
        AddFace(la, "底面", 40, 44, 4, 4,  mirror: "右臂·底面");
        ...
        AddFace(ll, "顶面", 20, 44, 4, 4,  mirror: "右腿·顶面");
        AddFace(ll, "底面", 24, 44, 4, 4,  mirror: "右腿·底面");
    }
    ...
    /// <summary>在 64x64 纹理上，位于某一面的镜像位置的像素坐标（水平翻转）。</summary>
